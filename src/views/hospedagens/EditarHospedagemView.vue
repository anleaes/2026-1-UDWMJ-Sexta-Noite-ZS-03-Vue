...
export default {
  name: 'EditarHospedagemView',
  components: { CaixaTexto },
  data() {
    const item = this.$route.query.item ? JSON.parse(this.$route.query.item) : {}
    return {
      titulo: item.titulo || '',
      descricao: item.descricao || '',
      tipo: item.tipo || '',
      endereco: String(item.endereco || ''),
      comodidadesSelecionadas: (item.comodidades || []).map(String),
      precoDiaria: String(item.preco_diaria || ''),
      capacidade: String(item.capacidade || ''),
      quartos: String(item.quartos || ''),
      banheiros: String(item.banheiros || ''),
  ...
  methods: {
    ...
    async salvar() {
      try {
        this.salvando = true
        await api.put(`/hospedagens/${this.itemId}/`, {
          titulo: this.titulo,
